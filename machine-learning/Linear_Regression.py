...
def gradientDescent(X,y,theta,alpha,num_iters):
    # 关键修正：把 ndarray 转成 matrix，这样后面用 X*theta 才是矩阵乘法
    X = np.matrix(X)
    y = np.matrix(y)
...
def computerCost(X,y,theta):
    # 关键修正：确保都是 matrix，避免 ndarray 下 * 变成逐元素乘法
    X = np.matrix(X)
    y = np.matrix(y)
    theta = np.matrix(theta)
...
def plotJ(J_history,num_iters):
    ...
    plt.xlabel(u"迭代次数") # 注意指定字体，要不然出现乱码问题
    plt.ylabel(u"代价值")
...
def predict(mu,sigma,theta):
    ...
    # 关键修正：np.ones((1)) 形状是 (1,) 会导致 hstack 报错；应为 (1,1)
